...
res = requests.get('https://www.cnuhh.com/medical/info/dept.cs?m=9', verify=False)
...
for i in dept_list :
    ...
    link = 'https://www.cnuhh.com/medical/info/dept.cs' + doct_dept['href']
...
for i in dept_link :
    ...
    for doc_link in real_link :
        doc_link = 'https://www.cnuhh.com/' + doc_link["href"]
        doc_num += 1
        res = requests.get(doc_link, verify=False)
        soup = BeautifulSoup(res.content, "html.parser", from_encoding='utf=8')
        wb = openpyxl.load_workbook('화순전.xlsx') 
